...
<%
    ...
    boolean isInstructor = "instructor".equalsIgnoreCase(__role);

    // FIX: Allow instructors — only block if neither admin nor instructor
    if (!isAdmin && !isInstructor) { response.sendRedirect(ctx + "/home"); return; }

    List<Instructor> instructorList = (List<Instructor>) request.getAttribute("instructorList");
    Instructor loggedInstructor     = (Instructor) request.getAttribute("loggedInstructor");
    List<Lesson> myLessons          = (List<Lesson>) request.getAttribute("myLessons");
    List<Test>   myTests            = (List<Test>)   request.getAttribute("myTests");
...
<head>
    ...
    <title><%= isAdmin ? "Instructors" : "My Dashboard" %> — DriveMaster</title>
    ...
    <style>
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .card{background:var(--bg2);border:1px solid var(--border);border-radius:16px;overflow:hidden;box-shadow:var(--shadow);animation:fadeUp .5s ease both;margin-bottom:24px}
        .card-header{padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px;font-size:14px;font-weight:700;color:var(--text)}
        ...
        .name-cell small{font-size:11px;color:var(--text3);display:block;margin-top:2px}
        .welcome-card{background:linear-gradient(135deg,rgba(245,158,11,.12),rgba(239,68,68,.08));border:1px solid rgba(245,158,11,.2);border-radius:16px;padding:24px 28px;margin-bottom:24px;animation:fadeUp .4s ease both}
        .welcome-title{font-size:20px;font-weight:800;color:var(--text);margin-bottom:4px}
...
<nav class="topbar">
    ...
    <div class="nav-links">
        ...
        <% } else { %>
        <a href="<%= ctx %>/instructors" class="nl active"><i class="fas fa-home"></i><span>Dashboard</span></a>
        <% } %>
    </div>
...
<div class="page">
    ...
    <div class="welcome-card">
        <div class="welcome-title"><i class="fas fa-hand-wave"></i> Welcome, <%= loggedInstructor.getName() %>!</div>
        <div class="welcome-sub">Specialization: <%= loggedInstructor.getSpecialization() %> &nbsp;|&nbsp; Experience: <%= loggedInstructor.getExperience() %> years</div>
    </div>
    <% } %>

    <%-- MY LESSONS --%>
    <div class="card">
        <div class="card-header"><i class="fas fa-calendar-check"></i> My Lessons</div>
        <% if (myLessons != null && !myLessons.isEmpty()) { %>
        <table class="data-table">
            <thead>
            <tr>
                <th>ID</th><th>Student</th><th>Date</th><th>Time</th>
                <th>Duration</th><th>Type</th><th>Vehicle</th><th>Status</th>
            </tr>
            </thead>
            <tbody>
            <% for (Lesson l : myLessons) { %>
            <tr>
                <td><span class="badge badge-blue"><%= l.getLessonId() %></span></td>
                <td><div class="name-cell"><strong><%= l.getStudentName() %></strong><small><%= l.getStudentId() %></small></div></td>
                <td><%= l.getLessonDate() %></td>
                <td><%= l.getLessonTime() %></td>
                <td><%= l.getDuration() %> hr</td>
                <td><span class="badge badge-yellow"><%= l.getLessonType() %></span></td>
                <td><%= l.getVehicleRegNumber() %></td>
                <td>
                    <span class="badge <%=
                        "Completed".equals(l.getStatus()) ? "badge-green" :
                        "Cancelled".equals(l.getStatus()) ? "badge-red"  : "badge-blue" %>">
                        <%= l.getStatus() %>
                    </span>
                </td>
            </tr>
        ...
        <% } else { %>
        <div class="empty-state"><i class="fas fa-calendar-check"></i><p>No lessons assigned yet.</p></div>
        <% } %>
    </div>

    <%-- MY TESTS --%>
    <div class="card">
        <div class="card-header"><i class="fas fa-clipboard-list"></i> My Tests (as Examiner)</div>
        <% if (myTests != null && !myTests.isEmpty()) { %>
        <table class="data-table">
            <thead>
            <tr>
                <th>ID</th><th>Student</th><th>Type</th><th>Date</th>
                <th>Time</th><th>Score</th><th>Result</th><th>Status</th>
            </tr>
            </thead>
            <tbody>
            <% for (Test t : myTests) { %>
            <tr>
                <td><span class="badge badge-purple"><%= t.getTestId() %></span></td>
                <td><div class="name-cell"><strong><%= t.getStudentName() %></strong><small><%= t.getStudentId() %></small></div></td>
                <td><span class="badge badge-yellow"><%= t.getTestType() %></span></td>
                <td><%= t.getTestDate() %></td>
                <td><%= t.getFormattedTime() %></td>
                <td><%= t.getScore() > 0 ? t.getScore() + "%" : "-" %></td>
                <td>
                    <span class="badge <%=
                        "Pass".equals(t.getResult()) ? "badge-green" :
                        "Fail".equals(t.getResult()) ? "badge-red"   : "badge-yellow" %>">
                        <%= (t.getResult() == null || t.getResult().isEmpty() || "Pending".equals(t.getResult())) ? "Pending" : t.getResult() %>
                    </span>
                </td>
                <td>
                    <span class="badge <%=
                        "Completed".equals(t.getStatus()) ? "badge-green" :
                        "Cancelled".equals(t.getStatus()) ? "badge-red"  : "badge-blue" %>">
                        <%= t.getStatus() %>
                    </span>
                </td>
            </tr>
        ...
        <% } else { %>
        <div class="empty-state"><i class="fas fa-clipboard-list"></i><p>No tests assigned yet.</p></div>
        <% } %>
    </div>
    ...
    <div class="page-header">
        <div>
            <div class="page-title"><i class="fas fa-chalkboard-teacher" style="color:var(--green)"></i> Instructor Management</div>
            <div class="page-sub">Track availability, qualifications and lessons</div>
        </div>
        <a href="<%= ctx %>/instructors?action=add-form" class="btn btn-primary"><i class="fas fa-plus"></i> Add Instructor</a>
    </div>

    <% if ("added".equals(success)) { %>
    <div class="alert alert-success"><i class="fas fa-check-circle"></i> Instructor added successfully!</div>
    <% } else if ("updated".equals(success)) { %>
    <div class="alert alert-success"><i class="fas fa-check-circle"></i> Instructor updated.</div>
    <% } else if ("deleted".equals(success)) { %>
    <div class="alert alert-success"><i class="fas fa-check-circle"></i> Instructor deactivated.</div>
    <% } else if ("nicExists".equals(error)) { %>
    <div class="alert alert-error"><i class="fas fa-exclamation-circle"></i> NIC already exists.</div>
    ...
    <div class="card">
        ...
        <table class="data-table">
            ...
            <tr>
                <td><span class="badge badge-blue"><%= i.getInstructorId() %></span></td>
                <td><div class="name-cell"><strong><%= i.getName() %></strong><small><%= i.getEmail() %></small></div></td>
                <td><%= i.getPhone() %></td>
                <td><span class="badge badge-yellow"><%= i.getSpecialization() %></span></td>
                <td><%= i.getExperience() %> yrs</td>
                <td>
                    <span class="badge <%= "Available".equals(i.getAvailability()) ? "badge-green" : "badge-red" %>">
                        <i class="fas fa-circle" style="font-size:6px"></i> <%= i.getAvailability() %>
                    </span>
                </td>
                <td><span class="badge <%= "ACTIVE".equals(i.getStatus()) ? "badge-green" : "badge-red" %>"><%= i.getStatus() %></span></td>
                <td><div class="actions">
                    <a href="<%= ctx %>/instructors?action=edit&id=<%= i.getInstructorId() %>" class="btn btn-sm btn-blue"><i class="fas fa-pen"></i> Edit</a>
                    <a href="<%= ctx %>/instructors?action=delete&id=<%= i.getInstructorId() %>" class="btn btn-sm btn-red"
                       onclick="return confirm('Deactivate this instructor?')"><i class="fas fa-ban"></i></a>
                </div></td>
            </tr>
            <% } %>
            </tbody>
        ...
        <% } else { %>
        <div class="empty-state"><i class="fas fa-chalkboard-teacher"></i><p>No instructors found.</p></div>
        <% } %>
    </div>
